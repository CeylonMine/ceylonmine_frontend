...
const ArrowIcon: React.FC<{ rotated: boolean }> = ({ rotated }) => (
  <svg
    className={`w-6 h-6 transform transition-transform duration-200 ${rotated ? 'rotate-180' : ''}`}
    fill="none"
    stroke="currentColor"
...
// Define a type for the success story
type SuccessStory = {
  title: string;
  region: string;
  license: string;
...
export default function LicensePortal() {
  ...
  useEffect(() => {
    ...
    // Listen for theme changes
    const themeChangeListener = (event: CustomEvent<{ isDarkMode: boolean }>) => {
      if (event.detail && event.detail.hasOwnProperty('isDarkMode')) {
        updateParticleColor();
      }
    };
      ...
      <main className="relative z-10 pt-28 pb-16">
        <div className="container mx-auto px-4">
          ...
          <div className="mt-24 mb-10">
            ...
            <div className={`rounded-xl overflow-hidden ${isDarkMode ? 'bg-gray-800/80' : 'bg-white/90'} border border-opacity-10 ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} shadow-lg backdrop-blur-sm max-w-4xl mx-auto p-6`}>
              {/* FAQ Item */}
              {[
                { 
